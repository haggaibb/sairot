...
class _AlonkaPageState extends State<AlonkaPage> {
  ...
  @override
  void dispose() {
    if (eventController.currentEvent.value.alonkaEndTime==null) _timer.cancel(); // Stop timer when widget is disposed
    super.dispose();
  }
            ...
            child: Center(
              child: Column(
                children: [
                        ...
                        _.currentEvent.value.alonkaSprints.length,
                          ...
                          return Padding(
                            ...
                            child: AlonkaRoundPanel(
                              round: _.currentEvent.value.alonkaSprints[index],
                            ),
                          );
                        }),
                  )
                  ...
                  Obx(() => eventController.loading.value
                      ? SizedBox(height: 100, width: 100,child: CircularProgressIndicator(),)
                      :  _.currentEvent.value.alonkaEndTime==null?ElevatedButton(
                      style: ElevatedButton.styleFrom(fixedSize: const Size(200, 40)),
                      onPressed: () async {
                        //setState(() async {
                          _.loading.value = true;
                          _.currentAlonkaRound.value = _.currentEvent.value.alonkaSprints.length;
                          if (_.currentEvent.value.alonkaSprints.isEmpty) eventController.currentEvent.value.alonkaStartTime = DateTime.now();
                          _.currentEvent.value.alonkaSprints.add(
                              AlonkaSprint(
                                  round: _.currentEvent.value.alonkaSprints.length,
                                  activeParticipants: _.currentEvent.value
                      ...
                      child: Text(
                        _.currentEvent.value.alonkaStartTime!=null?'התחל סיבוב חדש (צא)':'תחילת תרגיל',
                        style: TextStyle(fontSize: 14),
                      )):SizedBox.shrink()
                  ),
                  SizedBox(height: 100,),
                  _.currentEvent.value.alonkaEndTime==null && _.currentEvent.value.alonkaStartTime!=null
                      ? ElevatedButton(
                      ...
                      : _.currentEvent.value.alonkaEndTime!=null?Text('  התרגיל הסתיים  '):SizedBox.shrink(),
                  SizedBox(height: 80,),
